...
/**
 * The DTNReceive Activity. This Activity allows user to receive Text message
 * from DTN
 * 
...
public class DTNReceive extends Activity {
	 ...
	 * will associate the display view to dtnreceive layout and initialize the
	 * class
	 */
	...
	private void init() {
		init_UIs();
		bindDTNService();
		DestEIDEditText.setText(BundleDaemon.getInstance().local_eid().str()
				+ "/test");
	}
		...
		receiveButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					if (received_count > 0)
						new AlertDialog.Builder(DTNReceive.this)
								.setMessage(
										"Receive successfully "
												+ received_count + " messages")
								.setPositiveButton("OK", null).show();

					else {
						new AlertDialog.Builder(DTNReceive.this)
								.setMessage("Receive fail from timeout")
								.setPositiveButton("OK", null).show();
		...
		if (payload.location() == dtn_bundle_payload_location_t.DTN_PAYLOAD_MEM) {
			try {
				buf.append(String.format("Message: %s\n",
						new String(payload.buf(), "US-ASCII")));
	...
	private int receive_messages() throws DTNAPIFailException {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				try {
					do {
						receive_result = dtn_api_binder_.dtn_recv(handle,
								regid.intValue(), spec, dtn_payload, 1);
