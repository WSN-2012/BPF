...
public class EndpointID implements Serializable {
	...
	public boolean assign(byte[] array) {
		...
		try {
			...
			Logger.getInstance().error(TAG, "EndpointID:assign, unsupportedEncoding Exception");
			return false;
		}
	}
	...
	public boolean append_service_wildcard() {
		...
		if (!validate())
		{
			Logger.getInstance().error(TAG, "EndpointID::append_service_wildcard: failed to format appended URI");
			return false;
		}
	...
	public final singleton_info_t is_singleton() {
		if (!known_scheme())
		{
			Logger.getInstance().warning(TAG, "returning is_singleton_default= " + is_singleton_default_ + 
					 " for unknown scheme");
		}
	...
	protected boolean validate() {
		...
		{
			Logger.getInstance().debug(TAG,"EndpointID::validate: invalid URI");
			return false;
		}
		...
		if (scheme_str().length() > MAX_EID_PART_LENGTH) {
	        Logger.getInstance().error(TAG , "scheme name is too large > " +  MAX_EID_PART_LENGTH);
	        valid_ = false;
	        return false;
	    }
	    
	    if (ssp().length() > MAX_EID_PART_LENGTH) {
	    	Logger.getInstance().error(TAG, "ssp is too large (>" +  MAX_EID_PART_LENGTH);
	        valid_ = false;
	        return false;
	    }
