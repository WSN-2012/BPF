...
public class FragmentState {
	 ...
	 * 
	 * @param fragment
	 *            Fragment to add to the existing list.
	 * @return True if successfully added else false.
	 */
	public boolean add_fragment(Bundle fragment) {
		return fragments_.insert_sorted(fragment,
				BundleList.SORT_FRAG_OFFSET.getInstance());
	}
	...
	/**
	 * Function to check if bundle is complete or still some fragments are
	 * missing.
	 * 
	...
	final public boolean check_completed() {
		...
		try {
			...
			for (iter = fragments_.begin(); !iter.equals(fragments_.end()); ++fragi) {
				...
				assert (fragment.is_fragment()) : "FragmentState: Check Completed() fragment.is_fragment is false";
							...
							.debug(TAG,
									String.format(
											"check_completed fragment %d/%d: "
													...
													+ "(perfect fit)", fragi,
											fragn, f_offset, f_len, f_origlen,
											done_up_to));
					done_up_to += f_len;
				}
							...
							.debug(TAG,
									String.format(
											"check_completed fragment %d/%d: "
													+ "offset %d len %d total %d done_up_to %d: "
													+ "(redundant fragment)",
											fragi, fragn, f_offset, f_len,
											f_origlen, done_up_to));
					continue;
				}
