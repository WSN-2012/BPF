...
public class KeySteward 
{
			...
			IByteBuffer       enc_key) throws Exception
	{
	    ...
	    try {
	    	BPF.getInstance().getBPFLogger().debug(TAG, "Opening file: " + kspath);
	        fis = new FileInputStream(kspath);
	        BPF.getInstance().getBPFLogger().debug(TAG, "Loading keystore from: " + kspath);
...
//		BPF.getInstance().getBPFLogger().debug(TAG,"Got server's public key from certificate: 0x " + certKey_str);
