...
public class ProphetBundleRouter extends TableBasedRouter {
	...
	public ProphetBundleRouter(BundleActions actions, BundleList pendingBundles, BundleList custodyBundles) {
	...
	public void deliver_bundle(Bundle bundle) {
		...
		if (!bundle.payload().read_data(0, bundle.payload().length(), buf)) {
			BPF.getInstance().getBPFLogger()
					.error(TAG, "Erruor reading prophet bundle");
			return;
		}

		// BPF.getInstance().getBPFLogger().debug(TAG, toString(buf.array()));
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Bundle with length " + bundle.payload().length());
		...
		default:
			BPF.getInstance()
					...
					.error(TAG,
							String.format(
									"Unknown Prophet control(%x) from %s",
	...
	private void handleError(IByteBuffer buf, ProphetNeighbor controller,
			ProphetBundle hdr) {
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Received ERROR from " + controller.remote_eid());
	}
	...
	private void handleRIBDictionary(ProphetNeighbor pn, ProphetBundle hdr) {
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Received RIBDictionary from " + pn.remote_eid());
	...
	private void updateNeighborP_(ProphetNeighbor pn) {
		...
		while (sid.hasNext()) {
			...
			BPF.getInstance().getBPFLogger()
					.info(TAG, "transitivity updated " + p.P_() + " from " + p);
			...
			if (p.P_() <= P) {
				...
				BPF.getInstance()
						.getBPFLogger()
						.info(TAG,
								"Added route " + pn.remote_eid() + " - "
										+ p.P_() + " " + neid + " - " + P);
			} else {
				BPF.getInstance()
						.getBPFLogger()
						.info(TAG,
								"Skipped route " + pn.remote_eid() + " - "
										+ p.P_() + " " + neid + " - " + P);
			}
		}
	...
	private void notify(String s1, String s2) {
		BPF.getInstance().getBPFActionReceiver().notify(s1, s2);
	}

	private void handleBundleOffer(IByteBuffer buf, ProphetNeighbor pn,
			ProphetBundle hdr) {
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Received BundleOffer from " + pn.remote_eid());
	...
	private void sendBundleResponse(ProphetNeighbor pn) {
		...
		BPF.getInstance()
				.getBPFLogger()
				.debug(TAG,
						String.format("send Bundle Response %s",
								pn.remote_eid()));
	...
	private ProphetBundle parseProphetBundle(IByteBuffer buf) {
		...
		default:
			BPF.getInstance()
					.getBPFLogger()
					.error(TAG,
							"The bundle type was of unknown sort, something went terribly wrong");
			break;
		}
	...
	private void handleBundleResponse(IByteBuffer buf, ProphetNeighbor pn,
			ProphetBundle hdr) {
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Received Bundle Response from " + pn.remote_eid());
		reroute_all_bundles();
	}
	...
	private void sendError(ProphetNeighbor pn) {
		...
		sendMsg(adjustLenAndReturnArray(buf), pn);
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "send Error " + pn.remote_eid());
	}
	...
	private void sendHello(ProphetNeighbor pn, HelloFunctionType helloFun) {
		...
		BPF.getInstance()
				.getBPFLogger()
				.debug(TAG,
						String.format("send Hello(%s) %s",
								helloFun.getCaption(), pn.remote_eid()));
	...
	private void sendRIBInformation(ProphetNeighbor pn) {
		...
		sendMsg(adjustLenAndReturnArray(buf), pn);
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "send RIBInformation " + pn.remote_eid());
	...
	protected void handle_contact_up(ContactUpEvent event) {
		...
		if (remote_eid.equals(localEid())) {
			BPF.getInstance().getBPFLogger()
					.debug(TAG, "Link ID is equal self");
			return;
		}
		// Add new contact.
		if ((pn = neighbors.get(remote_eid)) == null) {
			...
			BPF.getInstance().getBPFLogger()
					.debug(TAG, "New neighbor " + pn.remote_eid());
		}
	...
	public void createBundleOfferTLV(IByteBuffer buf, ProphetNeighbor pn) {
		try {
			...
			while (bundles.hasNext()) {
				...
				BPF.getInstance()
						.getBPFLogger()
						.debug(TAG, b.bundleid() + " is offered to " + b.dest());
				// ID
