...
//      	byte_buffer_temp = BufferHelper.reserve(byte_buffer_temp, len);
        ...
        Logger.getInstance().debug(TAG, " primary_len: "+primary_len+" : len:"+len);
        ...
        if (primary.version() != BundleProtocol.CURRENT_VERSION) {
            Logger.getInstance().error(TAG, String.format("protocol version mismatch %s != %s",
        ...
        Logger.getInstance().debug(TAG, String.format("parsed primary block: version %s length %s",
        ...
        if (primary.creation_time_value() > Integer.MAX_VALUE) {
            Logger.getInstance().error(TAG, String.format("creation timestamp time is too large: %s",
        ...
        if (primary.creation_sequence_value() > Integer.MAX_VALUE) {
        	Logger.getInstance().error(TAG, String.format("creation timestamp sequence is too large: %s",
        ...
        if (primary.lifetime_value() > Integer.MAX_VALUE) {
            Logger.getInstance().error(TAG, String.format("lifetime is too large: %s", primary.lifetime));
            return -1;
        }
        ...
        if (len < primary.dictionary_length_value()) {
        	...
        	Logger.getInstance().error(TAG, String.format("primary block advertised incorrect length %s",
                      block.data_length()));
        ...
        if (buf_block_content.get((int) (buf_block_content.position()+primary.dictionary_length_value()-1)) != '\0') {
            Logger.getInstance().error(TAG, "dictionary does not end with a NULL character! "+primary_len);
            return -1;
        ...
        Logger.getInstance().debug(TAG, "Dict starting point :"+(primary_len-primary.dictionary_length_value()));
//        dictionary.position((int)(primary_len-primary.dictionary_length_value()));
        ...
        Logger.getInstance().debug(TAG, "Extract source :"+(primary_len-primary.dictionary_length_value()));
        ...
        if(!dict.extract_eid(bundle.source(),
        	...
        	Logger.getInstance().debug(TAG, "Extract source :"+bundle.source().str());
        }
        ...
        if(!dict.extract_eid(bundle.dest(),
        	...
        	Logger.getInstance().debug(TAG, "Extract dest :"+bundle.dest().str());
        }
        ...
        if(!dict.extract_eid(bundle.replyto(),
        	...
        	Logger.getInstance().debug(TAG, "Extract reply :"+bundle.replyto().str());
        }
        ...
        if(!dict.extract_eid(bundle.custodian(),
        	...
        	Logger.getInstance().debug(TAG, "Extract custodian :"+bundle.custodian().str());
        }
        ...
        if (bundle.is_fragment()) {
            ...
            if (sdnv_buf[0] > Integer.MAX_VALUE) {
                Logger.getInstance().error(TAG, String.format("fragment offset is too large: %s",
                          sdnv_buf));
                return -1;
            }
            ...
            if (sdnv_buf[0] > Integer.MAX_VALUE) {
                Logger.getInstance().error(TAG, String.format("fragment original length is too large: %s",
                          sdnv_buf));
                return -1;
            }
            ...
            Logger.getInstance().debug(TAG, String.format(TAG, "parsed fragmentation info: offset %s orig_len %s",
                        bundle.frag_offset(), bundle.orig_length()));
        }
        
        Logger.getInstance().debug(TAG, "primary_len: "+primary_len+" : ln"+len + ": Consumed"+consumed);
	...
	public boolean validate(final Bundle  bundle,
        ...
        if (bundle.source().equals(EndpointID.NULL_EID())) {
        	Logger.getInstance().debug(TAG, "Inside of Going to check null eid");
            ...
            if (bundle.custody_requested()) {
                Logger.getInstance().error(TAG,"bundle with null source eid has requested custody transfer; reject it");
                deletion_reason[0] = status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
                return false;
            }

            if (!bundle.do_not_fragment()) {
                Logger.getInstance().error(TAG,"bundle with null source eid has not set "
        ...
        if (bundle.is_admin()) {
            if (bundle.custody_requested()) {
                Logger.getInstance().error(TAG, "admin bundle requested custody transfer; reject it");
                deletion_reason[0] = status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
                return false;
            }
            ...
            {
                Logger.getInstance().error(TAG, "admin bundle has requested a report; reject it");
                deletion_reason[0] = status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
                return false;
            }
        }
    ...
    public void generate_primary(final Bundle bundle,
        ...
        Logger.getInstance().debug(TAG, String.format("generating primary: length %s", primary_len));
...
//        buf += dict->length();
    ...
    protected static int get_primary_len(final Bundle bundle,
        ...
        Logger.getInstance().debug(TAG, "get_primary_len: for bundleid = "+bundle.bundleid()+": "+primary_len);        
