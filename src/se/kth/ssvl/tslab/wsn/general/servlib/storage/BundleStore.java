...
public class BundleStore  {
    ...
    public boolean init(Context context, DTNConfiguration config){
    	...
    	Logger.getInstance().debug(TAG, "Total Valid Bundles: "+bundle_count_);
    ...
    public boolean add(Bundle bundle){
	    	try{
	    		...
	    		if(bundle.payload().location()==BundlePayload.location_t.DISK){
		    		Logger.getInstance().debug(TAG, "Going to add bundle in database with eids"+bundle.source().uri() );
    ...
    public Bundle get(int bundleid){
    	...
    	try{	
	    	...
	    	Logger.getInstance().debug(TAG, "Going to get bundle in database : "+bundleid );
    ...
    public boolean update(Bundle bundle){
    	...
    	if((resulte_is_record==id) && bundle.payload().location()==BundlePayload.location_t.DISK){
    		...
    		int bundleid = bundle.bundleid();
			Logger.getInstance().debug(TAG, "Going to update bundle in database : "+bundleid );		
			ContentValues values = new ContentValues();
			values.put("type", bundle.payload().location().getCode());
			...
			if(impt_sqlite_.update(table, values, condition, null)){
				if(!saved_bundles_.containsKey(bundle.bundleid())){
					...
					Logger.getInstance().debug(TAG, "Added size : "+ bundle.durable_size()+ " to "+ global_storage_.get_total_size());
				}
    ...
    public boolean del(Bundle bundle){
    	if(bundle.payload().location()==BundlePayload.location_t.DISK){
	    	...
	    	Logger.getInstance().debug(TAG, "Going to Del bundle in database: "+bundle.durable_key() );
	    	if(impt_sqlite_.delete_record(table,condition)){
				...
				if(saved_bundles_.containsKey(bundle.bundleid())){
					...
					Logger.getInstance().debug(TAG, "deleteing size : "+ bundle_size);
				}
    ...
    public boolean del(int bundleid){
	    	String condition = "id = "+bundleid;
	    	Logger.getInstance().debug(TAG, "Going to Del bundle in database:"+bundleid); 
	    	if(impt_sqlite_.delete_record(table,condition)){
	    		...
	    		Logger.getInstance().debug(TAG, "Going to Del bundle on disk:"+bundleid);
	    		bundle_count_ -= 1;
    ...
    public void clean_garbage_bundles(){
		...
		while(iterator.hasNext()){
			...
			if(b==null){
				//delete id
				Logger.getInstance().debug(TAG, "Validating Bundles deleting(bundle is null): "+id);
				this.del(id);
			}
			else{
				if(b.source().valid()){
					if(b.dest().valid()){
						Logger.getInstance().debug(TAG, "Source and dest EIDs validated: "+id);
						...
						Logger.getInstance().debug(TAG, "Only Source EID validated: "+id);
					}
				}
				else{
					Logger.getInstance().debug(TAG, "EIDs not validated: "+id);
				}
