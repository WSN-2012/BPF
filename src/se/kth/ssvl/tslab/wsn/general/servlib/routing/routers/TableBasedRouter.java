...
public abstract class TableBasedRouter extends BundleRouter {
	...
	protected void check_next_hop(Link next_hop) {
		...
		if (!next_hop.isopen()) {
			BPF.getInstance()
					...
					.debug(TAG,
							String.format(
									"check_next_hop %s -> %s: link not open...",
		...
		try {
			...
			BPF.getInstance().getBPFLogger()
					.error(TAG, "Table Based Router " + e.toString());
		} finally {
		...
		boolean add(final Bundle bundle, final ForwardingInfo info) {
			if (list_.contains(bundle)) {
				BPF.getInstance()
						...
						.error(TAG,
								String.format(
										"bundle %d already in deferred list!",
										bundle.bundleid()));
				return false;
			}
	...
	public void initialize() {
		Iterator<RoutesSetting.RouteEntry> itr = BPF.getInstance().getConfig()
	...
	protected void handle_bundle_received(BundleReceivedEvent event) {
		...
		if (!reception_cache_.add_entry(bundle, remote_eid)) {
			BPF.getInstance()
					...
					.info(TAG,
							String.format(
									"ignoring duplicate bundle: bundle %d",
									bundle.bundleid()));
			BundleDaemon
					.getInstance()
					.post_at_head(
	...
	protected void handle_contact_up(ContactUpEvent event) {
		...
		if (reroute_timer != null) {
			BPF.getInstance()
					...
					.debug(TAG,
							String.format(
									"link %s reopened, cancelling reroute timer",
									link.name()));
	...
	protected void handle_link_deleted(LinkDeletedEvent event) {
		...
		if (t != null) {
			BPF.getInstance()
					...
					.debug(TAG,
							String.format(
									"link %s deleted, cancelling reroute timer",
									link.name()));
			reroute_timers_.remove(link.name());
			t.cancel();
		}
