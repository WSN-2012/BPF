...
public class SQLiteImplementation {
    ...
    public int add(String table, ContentValues values){
        try{
			...
			Logger.getInstance().error(TAG, "SQLite Exception while adding a row");
			return -1;
		} 
    ...
    public boolean update(String table, ContentValues values, String where, String[] whereArgs){
        try{
			...
			Logger.getInstance().error(TAG, "SQLite Exception while updating a row");
		}
		return false;
	    	...
	    	if(cursor!=null){
	    		if (cursor.moveToFirst()){
		    		do{
		    			...
		    			Logger.getInstance().debug(TAG, "Found it@:"+cursor.getInt(idColumn));
    	...
    	try{
	    	...
	    	if(cursor!=null){
	    		if (cursor.moveToFirst()){
	    			...
	    			Logger.getInstance().debug(TAG, "Records count @:"+cursor.getInt(0));
	    		}
	    	}
	    	else{
	    		Logger.getInstance().debug(TAG, "Row not found!");
