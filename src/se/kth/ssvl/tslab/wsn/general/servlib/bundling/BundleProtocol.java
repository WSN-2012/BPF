...
public class BundleProtocol {
	...
	public static int generate_blocks(Bundle bundle, BlockInfoVec blocks,
	    ...
	    for (int i=0; i < blocks.size(); i++)
	    {
	        ...
	        Logger.getInstance().debug(TAG, String.format("generated block (owner %s type %s) "+
	...
	public static int produce(final Bundle bundle, final BlockInfoVec blocks,
		    ...
		    while (offset >= current_block.full_length()) {
		        ...
		        Logger.getInstance().debug(TAG, String.format("BundleProtocol::produce skipping block type %s " +
		    ...
		    while (true) {
		        ...
		        Logger.getInstance().debug(TAG, String.format("BundleProtocol::produce copying %d/%d bytes from " +
		    ...
		    Logger.getInstance().debug(TAG, String.format("BundleProtocol::produce complete: " +
	...
	public static int consume(Bundle bundle, IByteBuffer data, int len,
		    ...
		    if (recv_blocks.isEmpty()) {
		        Logger.getInstance().debug(TAG,  "consume: got first block... " +
		    ...
		    while (len != 0) {
		        Logger.getInstance().debug(TAG, String.format("consume: %d bytes left to process", len));
		        ...
		        if (info.complete()) {
		            ...
		            Logger.getInstance().debug(TAG, String.format("consume: previous block complete, " +
		        ...
		        Logger.getInstance().debug(TAG, String.format("consume: block processor %s type %s incomplete, " +
		        ...
		        if (cc < 0) {
		            Logger.getInstance().error(TAG, String.format("consume: protocol error handling block %s",
		                      info.type()));
		            return -1;
		        }
		        ...
		        Logger.getInstance().debug(TAG, String.format("consume: consumed %d bytes of block type %s (%s)",
		                    cc, info.type(),
		    ...
		    Logger.getInstance().debug(TAG, String.format("bundle id %d consume completed, %d/%d bytes consumed %s",
		                bundle.bundleid(),
		    		    origlen - len, 
		                origlen, 
	...
	public static boolean validate(Bundle bundle,
	    ...
	    if (recv_blocks.size() < 2) {
	        Logger.getInstance().error(TAG, "bundle fails to contain at least two blocks");
	    ...
	    if (recv_blocks.front().type() != BundleProtocol.bundle_block_type_t.PRIMARY_BLOCK) {
	        Logger.getInstance().error(TAG, "bundle fails to contain a primary block");
	        deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	        return false;
	    }
	    ...
	    for(int i=0; i < recv_blocks.size(); i++)
	    {
	        ...
	        if (current_block.data_offset() == 0) {
	            ...
	            if (i != last_block_index) {
	                Logger.getInstance().error(TAG, "bundle block too short for the preamble");
	                deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	                return false;
	            }
	            // "this is the last block, so drop it" [DTN2]
	            Logger.getInstance().debug(TAG, "forgetting preamble-starved last block");
	        ...
	        if (i == last_block_index) {
	            if (!current_block.last_block()) {
	                if (!bundle.fragmented_incoming()) {
	                    Logger.getInstance().error(TAG, "bundle's last block not flagged");
	                    deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	                    return false;
	                }
	                else {
	                    Logger.getInstance().debug(TAG, "bundle's last block not flagged, but " +
	                                     "it is a reactive fragment");
	                }
	            }
	        } else {
	            if (current_block.last_block()) {
	                Logger.getInstance().error(TAG, "bundle block incorrectly flagged as last");
	                deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	                return false;
	            }
	        }  	
	    ...
	    if (primary_blocks != 1) {
	        Logger.getInstance().error(TAG, String.format("bundle contains %d primary blocks", primary_blocks));
	        deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	        return false;
	    }
	    ...
	    if (payload_blocks > 1) {
	        Logger.getInstance().error(TAG, String.format("bundle contains %d payload blocks", payload_blocks));
	        deletion_reason[0] = BundleProtocol.status_report_reason_t.REASON_BLOCK_UNINTELLIGIBLE;
	        return false;
	    }
