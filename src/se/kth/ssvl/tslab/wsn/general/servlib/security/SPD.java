...
public class SPD 
{
	...
	public SPD()
	{
		BPF.getInstance().getBPFLogger().debug(TAG, "constructor: setting policy to SPD_USE_NONE");
			...
			spd_policy_t policy)
	{
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "SPD.set_global_policy() done" );
	...
	public boolean verify_in_policy( Bundle bundle)
	{
		...
		if ((policy.getCode() & spd_policy_t.SPD_USE_CB.getCode())>0) {
			if ( !Ciphersuite.check_validation(bundle, recv_blocks, Ciphersuite_C3.CSNUM_C3 )) {
				BPF.getInstance().getBPFLogger().debug(TAG, "SPD::verify_in_policy() no BP_TAG_CB_IN_DONE");
				return false;
			}
		}
		...
		if ((policy.getCode() & spd_policy_t.SPD_USE_PSB.getCode())>0) 
		{
			...
			{
				BPF.getInstance().getBPFLogger().debug(TAG, "SPD::verify_in_policy() no BP_TAG_PSB_IN_DONE");
				return false;
			}
		}
			...
			Bundle bundle)
	{
		...
		//(void)bundle;
		BPF.getInstance().getBPFLogger().debug(TAG, "SPD::find_policy()");
