...
public class GbofId {
	 ...
	 * @param creation_ts
	 *            Creation time of bundle
	 * @param is_fragment
	 *            True if bundle is a fragment else false
	 * @param frag_length
	 *            If fragment then fragment length
	 * @param frag_offset
	 *            If fragment then fragment offset
	 */
	public GbofId(EndpointID source, BundleTimestamp creation_ts,
			boolean is_fragment, long frag_length, long frag_offset) {
	 ...
	 * 
	 * @param id
	 *            Other GbofId object to compare
	...
	final public boolean equals(final GbofId id) {
		if (creation_ts_.seconds() == id.creation_ts().seconds()
				...
				&& (!is_fragment_ || (frag_length_ == id.frag_length_ && frag_offset_ == id.frag_offset_))
	 ...
	 * 
	 * @param source
	 *            Endpoint id to compare with the endpoint id of this
	 * @param bundleTimestamp
	 *            Bundle timestamp to compare with the timestamp of this
	 * @param is_fragment
	 *            Boolean is_frament flag to compare with the is_fragment of
	 *            this
	 * @param frag_length
	 *            Fragment frag_length to compare with the frag_lenght of this
	 * @param frag_offset
	 *            Fragment frag_offset to compare with the frag_offset of this
	 * @return True if all the fields match of this objects' fields else false
	 */
	final public boolean equals(EndpointID source,
			BundleTimestamp bundleTimestamp, boolean is_fragment,
			int frag_length, int frag_offset) {

		if (creation_ts_.seconds() == bundleTimestamp.seconds()
				&& creation_ts_.seqno() == bundleTimestamp.seqno()
				&& is_fragment_ == is_fragment
				&& (!is_fragment || (frag_length_ == frag_length && frag_offset_ == frag_offset))
	 ...
	 * 
	 * @param other
	 *            Other GbofId object to compare with this
	 * @return True if this is less than other else false
	 */
	final boolean isLessThan(final GbofId other) {
		if (source_.is_less_than(other.source()))
			return true;
		if (other.source().is_less_than(source_))
			return false;

		if (creation_ts_.isLessThan(other.creation_ts()))
			return true;
		if (creation_ts_.isGreaterThan(other.creation_ts()))
			return false;

		if (is_fragment_ && !other.is_fragment_)
			return true;
		if (!is_fragment_ && other.is_fragment_)
			return false;

		if (is_fragment_) {
			if (frag_length_ < other.frag_length_)
				return true;
			if (other.frag_length_ < frag_length_)
				return false;

			if (frag_offset_ < other.frag_offset_)
				return true;
			if (other.frag_offset_ < frag_offset_)
				return false;
		}
