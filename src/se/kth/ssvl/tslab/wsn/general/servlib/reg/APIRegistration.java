...
import se.kth.ssvl.tslab.wsn.general.servlib.bundling.BlockingBundleList;
...
public class APIRegistration extends Registration {
	 ...
	 * 
	 * @param regid
	 *            Registration id of the API Registration
	 * @param endpoint
	 *            Endpoint ID of host
	 * @param action
	 *            Action type
	...
	public APIRegistration(int regid, final EndpointIDPattern endpoint,
			failure_action_t action, int expiration) {
		...
		BPF.getInstance().getBPFLogger()
				.debug(TAG,
						String.format(
								...
								bundle.bundleid(), active() ? "active"
										: "deferred", endpoint_.str()));
	...
	/**
	 * This will block and wait other wise, put timeout in seconds This is for
	 * used in DTNAPI to get Bundle result from registration
	 * 
	 * @param timeout
	 *            Wait for the time before timeout, if timeout equal -1 it means
	 *            wait indefinite
