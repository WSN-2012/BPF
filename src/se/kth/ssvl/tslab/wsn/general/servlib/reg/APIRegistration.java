...
public class APIRegistration extends Registration{
	...
	public void deliver_bundle(Bundle bundle){
        ...
        if (!active() && (failure_action_ == failure_action_t.DROP)) {
            Logger.getInstance().debug(TAG, String.format("deliver_bundle: "
        ...
        if (!active() && (failure_action_ == failure_action_t.EXEC)) {
            ...
            // do -- it's in the spec
            Logger.getInstance().debug(TAG, String.format("deliver_bundle: running script '%s' for registration %s (%s)",
            			script_, regid_, endpoint_.str()));
        }

        Logger.getInstance().debug(TAG, String.format("deliver_bundle: queuing bundle id %d for %s delivery to %s",
                 bundle.bundleid(),
        ...
        if (active())
        {
        	...
        	{
        		Logger.getInstance().error(TAG, "Bundle List add fail because Illegal state exception");
        	}
        }
    }
