...
/**
 * This superclass represents a generic ciphersuite: it declares all the methods
 * that a specific ciphersuite must implement; it handles suite registration and
 * other general tasks.
 * 
 */
public class Ciphersuite {
	...
	public static boolean CS_FAIL_IF_NULL(BP_Local x) {
		if (x == null) {
			BPF.getInstance().getBPFLogger()
					.error(TAG, "error in CS_FAIL_IF_NULL");
			return true;
		} else
			return false;
	}
	...
	/**
	 * "Array of registered BlockProcessor-derived handlers for various
	 * ciphersuites -- fixed size for now [maybe make adjustable later]"[DTN2]
	 */
	...
	/**
	 * Values for security flags that appear in the ciphersuite flags field. Ex:
	 * HAS_SOURCE, HAS_DEST, HAS_PARAMS, HAS_PARAMS, HAS_CORRELATOR, has result.
	 * Defined in page 12 of security-draft15.
	 * 
	 */
	public static enum ciphersuite_flags_t {
		CS_BLOCK_HAS_SOURCE("Ciphersuite Block has source", (byte) 0x10), CS_BLOCK_HAS_DEST(
				"Ciphersuite Block has destination", (byte) 0x08), CS_BLOCK_HAS_PARAMS(
				"Ciphersuite Block has ciphersuite-parameters field",
				(byte) 0x04), CS_BLOCK_HAS_CORRELATOR(
				"Ciphersuite Block has correlator", (byte) 0x02), CS_BLOCK_HAS_RESULT(
				"Ciphersuite Block has result", (byte) 0x01), ;

		private static final Map<Byte, ciphersuite_flags_t> lookupCode = 
				new HashMap<Byte, ciphersuite_flags_t>();
		private static final Map<String, ciphersuite_flags_t> lookupCaption = 
				new HashMap<String, ciphersuite_flags_t>();
	...
	/**
	 * Values for flags that appear in the proc_flags_ field.
	 */
	public static enum proc_flags_t {
		CS_BLOCK_RESERVED0("Ciphersuite Block is reserved", (byte) 0x01), CS_BLOCK_PROCESSED(
				"Ciphersuite Block has been processed", (byte) 0x02), CS_BLOCK_DID_NOT_FAIL(
				"Ciphersuite Block did not fail", (byte) 0x04), CS_BLOCK_FAILED_VALIDATION(
				"Ciphersuite Block failed validation", (byte) 0x08), CS_BLOCK_PASSED_VALIDATION(
				"Ciphersuite Block passed validation", (byte) 0x10), CS_BLOCK_COMPLETED_DO_NOT_FORWARD(
				"Ciphersuite Block completed. Do not forward", (byte) 0x20), CS_BLOCK_PROCESSING_FAILED_DO_NOT_SEND(
				"Ciphersuite Block processing failed. Do not send", (byte) 0x40), ;
		...
		static {
			for (proc_flags_t s : EnumSet.allOf(proc_flags_t.class)) {
				lookupCode.put(s.getCode(), s);
				lookupCaption.put(s.getCaption(), s);
			}
		}
	...
	/**
	 * Types identifying security-parameter and security-result fields.
	 * Identifies the type in the tuple: type-length-value that represents each
	 * item. Ex: IV, key info, integrity signature, PCB ICV. Defined in Pag 21 of
	 * sec-draft15.
	 */
	public static enum ciphersuite_fields_t {
		CS_reserved0("CS_reserved0", (byte) 0x00), CS_IV_field("CS_IV_field",
				(byte) 0x01), CS_key_ID_field("CS_key_ID_field", (byte) 0x02), CS_encoded_key_field(
				"CS_encoded_key_field", (byte) 0x03), CS_fragment_offset_and_length_field(
				"CS_fragment_offset_and_length_field", (byte) 0x04), CS_signature_field(
				"CS_signature_field", (byte) 0x05), CS_reserved6(
				"CS_reserved6", (byte) 0x06), CS_C_block_salt(
				"CS_C_block_salt", (byte) 0x07), CS_C_block_ICV_field(
				"CS_C_block_ICV_field", (byte) 0x08), CS_reserved9(
				"CS_reserved9", (byte) 0x09), CS_encap_block_field(
				"CS_encap_block_field", (byte) 0x10), CS_reserved11(
				"CS_reserved11", (byte) 0x11);
	...
	public static void register_ciphersuite(Ciphersuite cs) {
		BPF.getInstance().getBPFLogger().debug(TAG, "register_ciphersuite()");
		...
		assert (ciphersuites_[num] == null);
		ciphersuites_[num] = cs;
	}

	public static Ciphersuite find_suite(long suiteNum) {
		Ciphersuite ret = null;
		BPF.getInstance().getBPFLogger().debug(TAG, "find_suite()");

		if (suiteNum > 0 && suiteNum < CS_MAX) // entry for element zero is
												// illegal
			...
			BPF.getInstance()
					.getBPFLogger()
					.error(TAG,
							"find_suite: Suite number: " + suiteNum
									+ ". out of bounds.");
		return ret;
	}
	...
	public static void init_default_ciphersuites() {
		if (!inited) {
			...
			BPF.getInstance()
					.getBPFLogger()
					.debug(TAG,
							"init_default_ciphersuites(): Processors and Ciphersuites registered.");
		} else {
			BPF.getInstance()
					.getBPFLogger()
					.error(TAG,
							"init_default_ciphersuites(): Processors and Ciphersuites are already registered!");
		}
	}
	...
	/**
	 * Called when a bundle is received. After BundleProtocol.consume has called
	 * Blockprocessor.consume to parse the preamble of the ASB, it calls this
	 * method to parse the rest of the ASB. It reads from the binary data inside
	 * block.contents() and puts it into the BP_LOCAL_CS class element (that
	 * represents an ASB). It extracts the ciphersuite-params and
	 * security-result from the raw data, and saves it into block.localcs,
	 * following the structure: type-length-value, type-length-value, ...
	 */
	public static void parse(BlockInfo block) {
		...
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Ciphersuite.parse() block");
		assert (block != null);
		...
		len -= sdnv_len;
		has_source = (cs_flags & ciphersuite_flags_t.CS_BLOCK_HAS_SOURCE
				.getCode()) != 0;
		has_dest = (cs_flags & ciphersuite_flags_t.CS_BLOCK_HAS_DEST.getCode()) != 0;
		has_params = (cs_flags & ciphersuite_flags_t.CS_BLOCK_HAS_PARAMS
				.getCode()) != 0;
		has_correlator = (cs_flags & ciphersuite_flags_t.CS_BLOCK_HAS_CORRELATOR
				.getCode()) != 0;
		has_result = (cs_flags & ciphersuite_flags_t.CS_BLOCK_HAS_RESULT
				.getCode()) != 0;

		BPF.getInstance()
				.getBPFLogger()
				.debug(TAG,
						"parse() suite_num " + suite_num + " cs_flags "
								+ cs_flags);

		cs_owner = find_suite(suite_num);

		if (ciphersuites_[(int) suite_num] != null) // get specific subclass if
													// it's present
			cs_owner = ciphersuites_[(int) suite_num];

		if (block.locals() == null) {
			if (cs_owner != null)
				cs_owner.init_locals(block); // get owning class to allocate
												// locals
			else {
		...
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "parse() correlator " + security_correlator);
		...
		if (has_params) {
			...
			len -= sdnv_len;
			locals.set_security_params(new SerializableByteBuffer(
					(int) field_length));

			BufferHelper.copy_data(locals.writable_security_params(), locals
					.writable_security_params().position(), buf,
					buf.position(), (int) field_length);

			buf.position((int) (buf.position() + field_length));

			len -= field_length;
			BPF.getInstance().getBPFLogger()
					.debug(TAG, "parse() security_params len " + field_length);
		}
		...
		BPF.getInstance()
				.getBPFLogger()
				.debug(TAG,
						"parse() eid_list().size() " + block.eid_list().size()
								+ " has_source: " + has_source + ". has_dest:"
								+ has_dest);
		...
		if (has_result) {
			...
			locals.set_security_result(new SerializableByteBuffer(
					(int) field_length));
			BPF.getInstance()
					.getBPFLogger()
					.debug(TAG,
							"sec-result length: " + sdnv_len + " + "
									+ field_length);
			len -= sdnv_len;
			// make sure the buffer has enough space, copy data in

			BufferHelper.copy_data(locals.writable_security_result(), locals
					.writable_security_result().position(), buf,
					buf.position(), (int) field_length);

			buf.position((int) (buf.position() + field_length));
			len -= field_length;
			BPF.getInstance().getBPFLogger().debug(TAG, "len final: " + len);
		}
	}
	...
	/**
	 * "Check the block list for validation flags for the specified
	 * ciphersuite"[DTN2]
	 */
	static boolean check_validation(final Bundle bundle,
			final BlockInfoVec block_list, int num) {
		short proc_flags = 0;
		BP_Local_CS locals;
		Iterator<BlockInfo> iter = block_list.iterator();

		BPF.getInstance().getBPFLogger()
				.debug(TAG, "Ciphersuite.check_validation(" + num + ")");
		while (iter.hasNext()) {
	...
	/**
	 * "Create a correlator for this block-list. Include part of the
	 * fragment-offset is this bundle is a fragment."[DTN2]
	 */
	static long create_correlator(final Bundle bundle,
		...
		BP_Local_CS locals;
		Iterator<BlockInfo> iter = block_list.iterator();
		BPF.getInstance().getBPFLogger().debug(TAG, "create_correlator()");
		if (bundle == null)
	...
	/**
	 * "Convenience methods to test if the security source/destination is an
	 * endpoint registered at the local node."[DTN2]
	 */
	static boolean source_is_local_node(final Bundle bundle,
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "source_is_local_node()");
		if (block == null)
			return false;
		...
		// this is a very clunky way to get the "base" portion of the bundle
		// destination
		...
		if (((cs_flags & Ciphersuite.ciphersuite_flags_t.CS_BLOCK_HAS_SOURCE
				.getCode()) > 0)
				&& (local_eid.toString().equals(locals.security_src()
						.toString()))
				|| !((cs_flags & Ciphersuite.ciphersuite_flags_t.CS_BLOCK_HAS_SOURCE
						.getCode()) > 0)
				&& (local_eid.toString().equals(src_node.toString()))) { // yes
																			// -
																			// this
																			// is
																			// ours
	 ...
	 * endpoint registered at the local node."[DTN2]
	 */
	static boolean destination_is_local_node(final Bundle bundle,
			final BlockInfo block) {
		short cs_flags = 0;
		EndpointID local_eid = BundleDaemon.getInstance().local_eid();
		BP_Local_CS locals;
		...
		BPF.getInstance().getBPFLogger()
				.debug(TAG, "destination_is_local_node()");
		if (block == null)
			return false;
		...
		if (((cs_flags & Ciphersuite.ciphersuite_flags_t.CS_BLOCK_HAS_DEST
				.getCode()) > 0)
				&& (local_eid.toString().equals(locals.security_dest()
						.toString()))
				|| !((cs_flags & Ciphersuite.ciphersuite_flags_t.CS_BLOCK_HAS_DEST
						.getCode()) > 0)
				&& (local_eid.toString().equals(dest_node.toString()))) { // yes
																			// -
																			// this
																			// is
																			// ours
	...
	void init_locals(BlockInfo block) {
		/*
		 * Create new locals block but do not overwrite if one already exists.
		 * Derived classes may wish to change this behavior and map old-to-new,
		 * or whatever
		 */
	 ...
	 * and content length." [DTN2]. It receives a block with loaded fields and
	 * generates the binary preamble in block.writablecontent and It creates the
	 * dictionary in xmit_blocks.dict().
	 * 
	 * @param xmit_blocks
	 * @param block
	 *            (IN/OUT): we walk over the block.eid list and add the eids
	 *            into the xmit_blocks.dict(). This method writes the binary
	 *            fields of the preamble into block.writablecontent
	 * @param type
	 *            (IN): type field, to be included into the preamble.
	 * @param flags
	 *            (IN): Block processing control flags to be included into the
	 *            preamble.
	 * @param data_length
	 *            (IN): data length to be included into the preamble.
	 * 
	 * 
	 */
	protected void generate_preamble(BlockInfoVec xmit_blocks, BlockInfo block,
		...
		block.owner().generate_preamble(xmit_blocks, block, type, flags,
				data_length);
	}
