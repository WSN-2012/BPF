...
import se.kth.ssvl.tslab.wsn.general.servlib.security.BA_BlockProcessor;
import se.kth.ssvl.tslab.wsn.general.servlib.security.BP_Local_CS;
import se.kth.ssvl.tslab.wsn.general.servlib.security.C_BlockProcessor;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite_BA1;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite_C3;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite_PS2;
import se.kth.ssvl.tslab.wsn.general.servlib.security.PS_BlockProcessor;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite.ciphersuite_fields_t;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite.ciphersuite_flags_t;
import se.kth.ssvl.tslab.wsn.general.servlib.security.Ciphersuite.proc_flags_t;
...
/**
 * This superclass represents a generic ciphersuite:
 * it declares all the methods that a specific ciphersuite must implement;
 * it handles suite registration and other general tasks.
 * 
 */
public class Ciphersuite 
{
	...
	public static boolean CS_FAIL_IF_NULL(BP_Local x)
	{
		if (x == null)
		{
			BPF.getInstance().getBPFLogger().error(TAG, "error in CS_FAIL_IF_NULL");
			return true;
		}
		else
			return false;
	...
	public static void init_default_ciphersuites()
	{
		if ( ! inited ) 
		{
			...
			inited = true;
			BPF.getInstance().getBPFLogger().debug(TAG, "init_default_ciphersuites(): Processors and Ciphersuites registered.");
		}
		else
		{
			BPF.getInstance().getBPFLogger().error(TAG, "init_default_ciphersuites(): Processors and Ciphersuites are already registered!");
		}
	}
	...
	public static void parse(BlockInfo block)
	{
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "Ciphersuite.parse() block");
		assert(block != null);
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "parse() suite_num " + suite_num + " cs_flags " + cs_flags);
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "parse() correlator " + security_correlator);
		...
		{    
			...
			len -= field_length;
			BPF.getInstance().getBPFLogger().debug(TAG, "parse() security_params len " + field_length);
		}

		//get sec-src length and data
		BPF.getInstance().getBPFLogger().debug(TAG, "parse() eid_list().size() " + block.eid_list().size() + " has_source: " + has_source + ". has_dest:" + has_dest);
		
		//pl - temp fix for blocks loaded from store
		...
		{    
			...
			BPF.getInstance().getBPFLogger().debug(TAG, "sec-result length: "+sdnv_len+" + "+field_length);
			len -= sdnv_len;
			...
			int            num)
	{
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "Ciphersuite.check_validation(" + num + ")");
		if ( block_list == null )
			return false;
	...
	static boolean destination_is_local_node(final Bundle    bundle, final BlockInfo block)
	{
		...
		BPF.getInstance().getBPFLogger().debug(TAG, "destination_is_local_node()");
		if ( block == null )
			return false;
