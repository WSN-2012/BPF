...
public class FragmentManager{
   ...
   public Bundle create_fragment(Bundle bundle,
        ...
        if ((offset + length) > fragment.orig_length()) {
            Logger.getInstance().error(TAG, String.format("fragment length overrun: "
    ...
    */
    public Bundle create_fragment(Bundle bundle,
        ...
        if (block_length > max_length) {
            Logger.getInstance().error(TAG, String.format("unable to create a fragment of length %s; minimum length "
        ...
        Logger.getInstance().debug(TAG, String.format("created %s byte fragment bundle with %s bytes of payload",
    ...
    public FragmentState proactively_fragment(Bundle bundle, 
       ...
       ListIterator<BlockInfo> entry = bundle.xmit_link_block_set().find_blocks(link).listIterator();
        ...
        Logger.getInstance().debug(TAG, String.format("proactively fragmenting "
    ...
    public void process_for_reassembly(Bundle fragment){
        ...
        Logger.getInstance().debug(TAG, String.format("processing bundle fragment id=%s hash=%s %s",
        ...
        if (iter == null) {
            ...
            state = iter;
            Logger.getInstance().debug(TAG, String.format("found reassembly state for key %s (%s fragments)",
        ...
        Logger.getInstance().debug(TAG, String.format("write_data: length_=%s src_offset=%s dst_offset=%s len %s",
    ...
    public void delete_obsoleted_fragments(Bundle bundle){
        ...
        Logger.getInstance().debug(TAG, String.format("checking for obsolete fragments id=%s hash=%s...",
                  bundle.bundleid(), hash_key[0]));
        
        if (state == null) {
            Logger.getInstance().error(TAG, String.format("no reassembly state for key %s",
                      hash_key[0]));
            return;
        }

        Logger.getInstance().debug(TAG, String.format("found reassembly state... deleting %d fragments",
                  state.num_fragments()));
