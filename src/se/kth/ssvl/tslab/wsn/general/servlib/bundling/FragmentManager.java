 ...
 * bundles, create bundle fragments from large bundles and reconstruct the whole
...
public class FragmentManager {
	 ...
	 * 
	 * @param bundle
	 *            the source bundle from which we create the fragment. Note: the
	 *            bundle may itself be a fragment
	 * @param offset
	 *            the offset relative to this bundle (not the original) for the
	 *            for the new fragment. note that if this bundle is already a
	 *            fragment, the offset into the original bundle will be this
	 *            bundle's frag_offset + offset
	 * @param length
	 *            the length of the fragment we want
	 * @return Newly created bundle
	 */
	public Bundle create_fragment(Bundle bundle, BlockInfoVec blocks,
		...
		if ((offset + length) > fragment.orig_length()) {
			BPF.getInstance().getBPFLogger().error(TAG,
							String.format(
									...
									fragment.frag_offset(), offset, length));
		...
		while (iter.hasNext()) {
			...
			if ((type == bundle_block_type_t.PRIMARY_BLOCK.getCode())
					...
					|| found_payload
					|| ((entry.flags() & block_flag_t.BLOCK_FLAG_REPLICATE
							.getCode()) > 0)) {
	 ...
	 * @param link
	 *            Link on which we will send the fragment
	 * @param blocks_to_copy
	 *            Blocks to copy on this fragment.
	 * @param offset
	 *            The offset relative to this bundle for the for the new
	 *            fragment
	 * @param max_length
	 *            Maximum length of the newly created fragment
	 * @return Newly created fragment.
	 */
	public Bundle create_fragment(Bundle bundle, final Link link,
			final BlockInfoVec blocks_to_copy, int offset, int max_length) {
		int block_length = 0;
		...
		// initialize payload
		int to_copy = Math.min(max_length - block_length, bundle.payload()
				.length() - offset);
		fragment.payload().set_length(to_copy);
		fragment.payload().write_data(bundle.payload(), offset, to_copy, 0);
		BlockInfoVec xmit_blocks = fragment.xmit_link_block_set()
				.create_blocks(link);
	...
	/**
	 * Given the given fragmentation threshold, determine whether the given
	 * bundle should be split into several smaller bundles. If so, this returns
	 * true and generates a bunch of bundle received events for the individual
	 * fragments.
	 * 
	 * @param bundle
	 *            Bundle to split into fragments
	 * @param link
	 *            Bundle to send on the given link
	 * @param max_length
	 *            Maximum length of the fragment
	 * @return FragmentState that has the list of created fragments for the
	 *         given bundle.
	 */
	public FragmentState proactively_fragment(Bundle bundle, final Link link,
			int max_length) {
	...
	/**
	 * Given a newly arrived bundle fragment, append it to the table of
	 * fragments and see if it allows us to reassemble the bundle. If it does, a
	 * ReassemblyCompletedEvent will be posted.
	 * 
	 * @param fragment
	...
	public void process_for_reassembly(Bundle fragment) {
		...
		assert (fragment.is_fragment()) : TAG
				+ ": process_for_reassembly() not a fragment";
		...
		BPF.getInstance().getBPFLogger()
				.debug(TAG,
						String.format(
								...
								state.bundle().payload().length(), 0,
								fragment.frag_offset(), fraglen));
		...
		if (fragment.frag_offset() == 0
			...
			ListIterator<BlockInfo> entry = fragment.recv_blocks()
					.listIterator();
		...
		BundleDaemon.getInstance().post_at_head(
				new ReassemblyCompletedEvent(state.bundle(), state
						.fragment_list()));
		assert (state.fragment_list().size() == 0) : TAG
				+ ": process_for_reassembly size not 0"; // moved into the event
		fragment_table_.remove(hash_key);
	}

	/**
	 * Delete any fragments that are no longer needed given the incoming
	 * (non-fragment) bundle.
	 */
	public void delete_obsoleted_fragments(Bundle bundle) {
		...
		try {
			...
			assert (state.fragment_list().size() == 0) : TAG
					+ ": delete_obsoleted_fragments Size not 0"; // moved into
																	// events
	...
	public void delete_fragment(Bundle fragment) {
		...
		assert (fragment.is_fragment()) : TAG
				+ "delete_fragment() not a fragment";
	 ...
	 * 
	 * @param bundle
	 *            Bundle to find the find key
	 * @param key
	 *            Save the key on the first index of the String Array
	 */
	protected void get_hash_key(final Bundle bundle, String[] key) {
		String temp;

		temp = String.format("%s.%s", bundle.creation_ts().seconds(), bundle
				.creation_ts().seqno());
