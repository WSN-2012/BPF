...
/**
 * 
 * Class to encapsulate bundle forwarding information. This is created when a
 * bundle is forwarded to log a record of the forwarding event, along with any
 * route-specific information about the action, such as the custody timer.
 * 
...
public class ForwardingInfo implements Serializable {
	...
	/**
	 * Convenience flag to specify any forwarding action for use in searching
	 * the log.
	 */

	public enum action_t {
		INVALID_ACTION(0), FORWARD_ACTION(1 << 0), // /< Forward the bundle to
													// only this next hop
		COPY_ACTION(1 << 1)// /< Forward a copy of the bundle
		;

		private static final Map<Integer, action_t> lookup = new HashMap<Integer, action_t>();
	 ...
	 * 
	 * @param action
	 *            Action object which needs to convert
	...
	/**
	 * Convenience flag to specify any forwarding state for use in searching the
	 * log.
	 */
	 ...
	 * 
	 * @param state
	 *            State_t object which needs to convert
	 ...
	 * 
	 * @param state
	 *            Set State of ForwardingInfo
	 * @param action
	 *            Set Action to perform of ForwardingInfo
	 * @param link_name
	 *            Link name
	 * @param regid
	 *            Set registration id of ForwardingInfo
	 * @param remote_eid
	 *            Set remote eid of ForwardingInfo
	 * @param custody_spec
	 *            Set Set custody timer of ForwardingInfo
	 */

	public ForwardingInfo(state_t state, action_t action, String link_name,
			int regid, final EndpointID remote_eid,
			final CustodyTimerSpec custody_spec) {
		state_ = state_t.NONE;
		action_ = action;
	 ...
	 * 
	 * @param new_state
	 *            New state type to update old state of ForwardingInfo
	 */
