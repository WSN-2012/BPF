...
public class BlockProcessor implements Serializable {
	...
	public int consume(Bundle bundle,  BlockInfo block,  IByteBuffer buf,  int len)
     {
        ...
        Logger.getInstance().debug(TAG, String.format( "BlockProcessor type %s " +
                          ...
                          BundleProtocol.status_report_reason_t[] deletion_reason)
    {
        ...
        if (bundle.is_admin() &&
            ...
            Logger.getInstance().error(TAG, String.format("invalid block flag %s for received admin bundle",
			...
			IByteBuffer buf, int len, int[] processing_flags) 
	{
		    ...
		    if (block_len[0] > Integer.MAX_VALUE) {
		        Logger.getInstance().error(TAG, String.format("overflow in SDNV value for block type %s",
		                  block.type().toString()));
		        return -1;
		    }
