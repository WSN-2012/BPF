...
public class PayloadBlockProcessor extends BlockProcessor implements
		Serializable {
	...
	/**
	 * This function consumes the payload block of the bundle. It is a virtual
	 * from BlockProcessor.
	 * 
	 * @param bundle
	 *            Bundle to set data after consuming
	 * @param blcok
	 *            Payload block to set data after consuming
	 * @param buf
	...
	public int consume(Bundle bundle, BlockInfo block, IByteBuffer buf, int len) {
		...
		if (block.data_offset() == 0) {
			int cc = super
					.consume_preamble(recv_blocks, block, buf, len, flags);
			if (cc == -1) {
				return -1;
			}
			...
			assert (bundle.payload().length() == 0) : TAG
		...
		if (bundle.payload().location().getCode() == location_t.NODATA
				.getCode()) {
		...
		assert (block.contents().capacity() == block.data_offset()) : TAG
				+ ": consume() data_offset not equal to content capacity";
	 ...
	 * 
	 * @param bundle
	 *            Bundle to generate preamble
	 * @param xmit_blocks
	 *            xmit_blocks to get the dictionary for generating preamble
	 * @param block
	 *            Block to write the payload preabmple
	 * @param link
	 *            Link type
	 * @param last
	 *            If its a last block
	...
	public int generate(final Bundle bundle, BlockInfoVec xmit_blocks,
		...
		super.generate_preamble(xmit_blocks, block,
				bundle_block_type_t.PAYLOAD_BLOCK,
				last ? block_flag_t.BLOCK_FLAG_LAST_BLOCK.getCode() : 0, bundle
						.payload().length());
	...
	/**
	 * Produce a bundle by coping all payload and payload block metadata to
	 * buffer.
	 * 
	 * @param bundle
	 *            Bundle to produce
	 * @param block
	 *            Payload block of Bundle
	 * @param buf
	 *            Copy the data on the current position this buffer
	 * @param offset
	 *            Offset of the block data. Keep track of how many bytes have
	...
	public void produce(final Bundle bundle, final BlockInfo block,
		...
		if (offset < block.data_offset()) {
			...
			BufferHelper.copy_data(buf, buf.position(), block.contents(),
					offset, tocopy);
