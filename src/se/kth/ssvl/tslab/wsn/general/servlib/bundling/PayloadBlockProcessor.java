...
public class PayloadBlockProcessor extends BlockProcessor implements Serializable{
	...
	public int consume(Bundle    bundle,
        ...
        if (block.data_offset() == 0) {
            ...
            len -= cc;
            consumed += cc;
            Logger.getInstance().debug(TAG, "in len: "+len+" : cc"+cc+" "+ bundle.payload().length());
            assert(bundle.payload().length() == 0)
            :TAG+"consume() bundle payload length is not 0";
        }        
        
        Logger.getInstance().debug(TAG, "out len: "+len+" : cc"+consumed+" "+ bundle.payload().length());
        ...
        Logger.getInstance().debug(TAG, String.format("consumed %s/%s (%s)",
	...
	public boolean validate(final Bundle           bundle,
        ...
        if (!block.complete()) {
            ...
            {
                Logger.getInstance().debug(TAG, "payload incomplete and cannot be fragmented");
