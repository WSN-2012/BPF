 ...
 * specification. Every string is separated by null value.
 * 
...
public class Dictionary implements Serializable {
	...
	/**
	 * Set the dictionary from a buffer of size length. Take the data from
	 * current position of buffer of size length and copy in new dictionary.
	 * 
	 * @param buff
	 *            Serializable buffer populated with data
	 * @param length
	 *            Number of bytes copy from buff.
	 */
	 ...
	 * 
	 * @param eid
	 *            EndpointID to add in dictionary
	 */
	...
	/**
	 * Find given string in dictionary. If found set the offset of string and
	 * return.
	 * 
	 * @param find
	 *            String value converted to a byte array
	 * @param offset
	 *            Set offset on the first index of offset array if given string
	 *            found.
	...
	/**
	 * Find given endpoint id in dictionary. If found set the scheme_offset &
	 * ssp_offset and return true.
	 * 
	 * @param eid
	 *            EndpointID to find in dictionary
	 * @param scheme_offset
	 *            Set scheme_offset on the first index of scheme_offset array if
	 *            scheme part of endpoint id is found.
	 * @param ssp_offset
	 *            Set ssp_offset on the first index of ssp_offset array if ssp
	 *            part of endpoint id is found.
	 * @return True if given endpoint id found in dictionary else false.
	 */

	public boolean get_offsets(final EndpointID eid, int[] scheme_offset,
			int[] ssp_offset) {

		return (get_offset(eid.scheme_str().getBytes(), scheme_offset) && get_offset(
				eid.ssp().getBytes(), ssp_offset));
	}
	...
	public boolean get_offsets(final EndpointID eid, long[] scheme_offset,
			long[] ssp_offset) {
		if (eid.valid()) {
			return (get_offset(eid.scheme_str().getBytes(), scheme_offset) && get_offset(
					eid.ssp().getBytes(), ssp_offset));
	 ...
	 * 
	 * @param eid
	 *            Empty object of EndpointId
	 * @param scheme_offset
	 *            Scheme offset of eidpoint id
	 * @param ssp_offset
	 *            Ssp offset of eidpoint id
	 ...
	 *            Empty object of EndpointId
	 * @param scheme_offset
	 *            Scheme offset of eidpoint id in the first index of array
	 * @param ssp_offset
	 *            Ssp offset of eidpoint id in the first index of array
	 * @return True on if endpoint id created successfully else false.
	 */

	public boolean extract_eid(EndpointID eid, long[] scheme_offset,
			long[] ssp_offset) {
	...
	public String get_value(int offset_val) {
		...
		if (get_offset(separator, offset) && offset[0] > offset_val) {
