...
public class Dictionary implements Serializable {
    ...
    public boolean extract_eid(EndpointID eid,int scheme_offset, int ssp_offset){
        ...
        if (dict_length_ == 0) {
            Logger.getInstance().error(TAG, "cannot extract eid from zero-length dictionary");
            return false;
        }    
    	
        if (scheme_offset >= (dict_length_ - 1)) {
        	Logger.getInstance().error(TAG, String.format("illegal offset for scheme dictionary offset: "
                    +"offset %d, total length %s", scheme_offset, dict_length_));
        	return false;
        }

        if (ssp_offset >= (dict_length_ - 1)) {
        	Logger.getInstance().error(TAG, String.format("illegal offset for ssp dictionary offset: "
