...
public class RouteTable{
	    ...
	    public boolean add_entry(RouteEntry entry)
	    {
	    	...
	    	try
	    	{
	    		Logger.getInstance().debug(TAG, String.format(" add_route %s", entry.toString()));
	    ...
	    public boolean del_entry(final EndpointIDPattern dest, final Link next_hop)
	    {
	    	...
	    	try
	    	{
	    		...
	    		while (iter.hasNext())
	    		{
	    			...
	    			{
	    				Logger.getInstance().debug(TAG, String.format("del_entry %s", entry.toString()));
	    				iter.remove();
	    				return true;
	    			}
	    		}
	    		Logger.getInstance().debug(TAG, String.format("del_entry %s -> %s: no match!",
	   ...
	   public int del_matching_entries(RouteEntryMatcher matcher)
	    		...
	    		final Link next_hop, RouteEntryVec entry_vec) 
	    {
	    	...
	    	try
	    	{
	    		...
	    		loop[0] = false;
	    		Logger.getInstance().debug(TAG, String.format("get_matching %s (link %s)...",
	    				eid.uri().toString(), 
	    	    ...
	    	    if (loop[0]) {
	    	        Logger.getInstance().warning(TAG, String.format("route destination %s caused route table lookup loop",
	    	                 eid.uri().toString()));
	    	    }
	                               ...
	                               int               level) 
	    {
	    	...
	    	try
	    	{
	    		...
	    		while (iter.hasNext())
	    		{
	    			...
	    			Logger.getInstance().debug(TAG, String.format("check entry %s", entry.toString()));
	    			...
	    			else if ( next_hop == null || entry.link().equals(next_hop))
	    			{
	    				...
	    				{
	    					Logger.getInstance().debug(TAG,String.format("match entry %s", entry.toString() ));
	    					entry_vec.add(entry);
	    					++count;
	    				}
	    				else
	    				{
	    					Logger.getInstance().debug(TAG,String.format("entry %s already in matches... ignoring", entry.toString() ));
	    				}
	    		...
	    		Logger.getInstance().debug(TAG, String.format("get_matching %s done (level %d), %d match(es)",
