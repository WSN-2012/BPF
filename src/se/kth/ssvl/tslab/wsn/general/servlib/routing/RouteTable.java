 ...
 * check what route entries should be used for particular destination.
 * 
...
public class RouteTable {
	...
	/**
	 * "Fill in the RouteEntryVec with the list of all entries whose patterns
	 * match the given eid and next hop." [DTN2] If the next hop is null, it is
	 * ignored.
	 * 
	 * @return the count of matching entries
	 */
	public int get_matching(final EndpointID eid, final Link next_hop,
			...
			if (loop[0]) {
				BPF.getInstance().getBPFLogger()
						.warning(
								TAG,
								String.format(
										"route destination %s caused route table lookup loop",
										eid.uri().toString()));
			}
	...
	public void dump(StringBuffer buf) {
		...
		try {
			...
			while (itr.hasNext()) {
				...
				dest_eid_width = Math.max(dest_eid_width, e.dest_pattern()
						.length());
				source_eid_width = Math.max(source_eid_width, e
						.source_pattern().length());
				next_hop_width = Math.max(next_hop_width,
						(e.link() != null) ? e.link().name().length() : e
								.route_to().length());
			...
			RouteEntry.dump_header(buf, dest_eid_width, source_eid_width,
					next_hop_width);
			...
			while (itr.hasNext()) {
				RouteEntry e = itr.next();
				e.dump(buf, long_strings, dest_eid_width, source_eid_width,
						next_hop_width);
	...
	/**
	 * Helper function for get_matching. It's a recursive function to get the
	 * actual RouteEntry
	 * 
	 * @param eid
	 *            EndpointID to find
	 * @param next_hop
	 *            The next hop link of the route
	 * @param entry_vec
	 *            the entry_vec to fill up
	 * @param loop
	 *            sanity flag checker whether the running system is looping or
	 *            not
	 * @param level
	 *            the current depth level of this recursive call
	 * @return the number of matching
	 */
	protected int get_matching_helper(final EndpointID eid,
			final Link next_hop, RouteEntryVec entry_vec, boolean[] loop,
			int level) {
		lock_.lock();
		try {
			int count = 0;
			...
			while (iter.hasNext()) {
				...
				// entry.link() .equals next hop when we were adding next_hop
				// style" [DTN2]
